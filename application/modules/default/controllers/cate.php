...
class cate extends CI_Controller {
	...
	public function megamenu(){
		...
		if(!empty ($SortedList)){
			...
			$data['info'] = $meta;
			$data['template'] = "";
			$this->load->view('layout/megamenu', $data);
		...
		if ($this->input->post ( "ok" )) {
			...
			if ($this->form_validation->run ()) {
				...
				foreach ( $listall as $row ) {
					if (in_array ( trim ( $cate_name ), $row ) && $row ['cate_id'] != $id)
						$data ['errorName'] = "Đã tồn tại";
					if ($row ['cate_parent'] == $id && $this->check ( $id, $parentname ) == 1)
						$data ['errorTrung'] = "Không được chọn con của nó";
					// check bi lap vo tan
				}
				if ($parentname == $rows ['cate_id'])
					$data ['errorTrung'] = "Không được chọn";
